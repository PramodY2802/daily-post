...
export function formatReportText(report: AnalyticsReport): string {
  let text = `📊 *${report.period} Report*\n\n`;
  text += `📝 Total Posts: ${report.totalPosts}\n`;
  text += `❤️ Total Likes: ${report.totalLikes}\n`;
  text += `💬 Total Comments: ${report.totalComments}\n`;
  text += `🔄 Total Shares: ${report.totalShares}\n`;
  text += `👁️ Total Impressions: ${report.totalImpressions}\n`;
  text += `📈 Avg. Engagement: ${report.avgEngagementRate.toFixed(2)}%\n`;
