...
export async function createVideoContent(request: ContentRequest): Promise<VideoContentResult> {
  ...
  logger.info('Video pipeline â€” text generated', { length: textContent.text.length });

  // Step 2: TTS â€” convert text to audio (voice from project config, else picked by language)
  ...
  logger.info('Video pipeline â€” TTS completed', { audioPath });
  ...
  try {
    ...
    logger.info('Video pipeline â€” AI image generated', { imagePath });
  } catch (err) {
    ...
    if (pexelsImages.length === 0) {
      // Keep video generation usable even when external image providers are unavailable.
      const isVertical = request.contentType === ContentType.SHORT || request.contentType === ContentType.REEL;
      imagePath = createFallbackBackground(isVertical ? 1080 : 1920, isVertical ? 1920 : 1080);
      logger.info('Video pipeline - local fallback background created', { imagePath });
    } else {
...
function createFallbackBackground(width: number, height: number): string {
  ensureDir(TEMP_DIR);
  const filePath = path.join(TEMP_DIR, `fallback_${randomUUID()}.ppm`);
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      const xMix = x / Math.max(1, width - 1);
      const i = (y * width + x) * 3;
      pixels[i] = Math.round(28 + 42 * xMix);
      pixels[i + 1] = Math.round(18 + 78 * yMix);
      pixels[i + 2] = Math.round(38 + 92 * (1 - xMix * yMix));
    }
  }
